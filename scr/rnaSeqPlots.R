...
tot %>% filter(FDR < 0.05 & PValue < 0.05) %>% mutate(state = ifelse(logFC > -1.5 & logFC < 1.5 , "unchanged", "deregulated" )) %>% mutate(trueS = ifelse(is.na(SYMBOL), paste(ensemblId), paste(SYMBOL))) %>% mutate(labels = ifelse(state == "deregulated", paste(trueS), "")) %>% mutate(logPval = -log10(PValue), high = ifelse(logPval == "Inf", 350, paste(logPval))) %>% ggplot(aes(logFC, as.numeric(high), color = state)) + geom_point() +  theme_bw() + geom_vline(xintercept=c(-1.5, 1.5), col="grey") + geom_hline(yintercept=-log10(0), col="grey") +  scale_color_manual(values = mycol) + geom_text_repel(aes(label = labels), max.overlaps = 30) + ggtitle(paste(args[3], "FDR <0.05, PValue <0.05, logFC< -1.5 & > 1.5", sep = " ")) + ylab("-log10(PValue)")
