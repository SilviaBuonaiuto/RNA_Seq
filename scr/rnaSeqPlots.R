...
new<-tot %>% filter(FDR < 0.5 & PValue < 0.05) %>% mutate(state = ifelse(logFC > -1 & logFC < 1 , "unchanged", "deregulated" )) %>% mutate(trueS = ifelse(is.na(SYMBOL), paste(ensemblId), paste(SYMBOL))) %>% mutate(labels = ifelse(state == "deregulated", paste(trueS), ""))
final<-merge(new, counts, by = "ensemblId")
mydf<-final %>% filter(abs(logFC) > 1 | abs(logFC)< -1 & FDR<0.5 & PValue < 0.05) %>% select(trueS, 12:19)
mat<-as.matrix(mydf[,2:9])
